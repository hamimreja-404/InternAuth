...
//     const fetchTeachers = async () => {
// try {
//         // Just call the function. Our api.js wrapper already returns the data directly!
...
import { 
  Users, Mail, School, BookOpen, Calendar, 
  Loader2, Search, UserCircle, Phone
} from 'lucide-react';
...
export default function TeachersTable() {
  ...
  // Filter logic safely checking nested auth_user data
  const filteredTeachers = teachers.filter((teacher) => {
    const firstName = teacher.auth_user?.first_name || '';
    const lastName = teacher.auth_user?.last_name || '';
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 relative overflow-hidden font-sans">
      <div className="relative z-10 max-w-7xl mx-auto">
        
        {/* Header Section */}
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            <h2 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
              Teachers Directory
            </h2>
          ...
          <div className="relative w-full sm:w-auto">
            ...
            <input 
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full sm:w-64 bg-white/5 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-white placeholder-slate-500 focus:bg-white/10 outline-none"
            />
          </div>
        </motion.div>

        {/* Table Container */}
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-4xl shadow-2xl overflow-hidden">
          <div className="overflow-x-auto p-6">
            <AnimatePresence mode="wait">
              {loading ? (
                <motion.div key="loader" exit={{ opacity: 0 }} className="flex flex-col items-center justify-center py-20 gap-4">
                  <Loader2 className="w-10 h-10 text-purple-400 animate-spin" />
                  <p className="text-slate-400 animate-pulse">Fetching Secure Data...</p>
                </motion.div>
              ) : (
                <motion.table key="table" variants={tableVariants} initial="hidden" animate="show" className="w-full text-left border-collapse">
                  <thead>
                    <tr>
                      <th className="pb-4 text-xs font-semibold text-slate-400 uppercase border-b border-white/10 px-4">Name</th>
                      <th className="pb-4 text-xs font-semibold text-slate-400 uppercase border-b border-white/10 px-4">Contact Info</th>
                      <th className="pb-4 text-xs font-semibold text-slate-400 uppercase border-b border-white/10 px-4">University</th>
                      <th className="pb-4 text-xs font-semibold text-slate-400 uppercase border-b border-white/10 px-4">Dept.</th>
                      <th className="pb-4 text-xs font-semibold text-slate-400 uppercase border-b border-white/10 px-4">Gender</th>
                      <th className="pb-4 text-xs font-semibold text-slate-400 uppercase border-b border-white/10 px-4">Joined</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {filteredTeachers.map((teacher) => (
                      <motion.tr key={teacher.id} variants={rowVariants} className="hover:bg-white/5 transition-colors">
                        
                        {/* Avatar & Name */}
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-purple-500/20 text-purple-300 flex items-center justify-center text-xs font-bold">
                              {teacher.auth_user?.first_name?.[0] || ''}{teacher.auth_user?.last_name?.[0] || ''}
                            </div>
                        ...
                        <td className="py-4 px-4 text-sm text-slate-300">
                          <div>{teacher.auth_user?.email}</div>
                          <div className="text-slate-500 text-xs mt-1 flex items-center gap-1">
                            <Phone className="w-3 h-3"/> {teacher.auth_user?.mobile || 'N/A'}
                          </div>
                        </td>

                        {/* Academic Info */}
                        <td className="py-4 px-4 text-sm text-slate-300">{teacher.university}</td>
                        <td className="py-4 px-4">
                          <span className="px-2 py-1 rounded-full text-xs bg-indigo-500/10 text-indigo-300">
                            {teacher.department || 'N/A'}
                          </span>
                        </td>
                        <td className="py-4 px-4 text-sm text-slate-300">{teacher.gender}</td>
                        <td className="py-4 px-4 text-sm text-slate-400">{teacher.year_of_join}</td>
